<?php
    ...
    $player = new PlayerController(DbConnection::connect());
        ...
        $player_data = PlayerController::getById();
    ...
    if($player->validatePlayerData($_POST))
    {
        if($player->update())
        {
            echo "<h1>est modifier</h1>";
        }
        else
        {
            echo "<h1>Erreur de modefication modifier</h1>";
        }
    ...
    <div class="max-w-md mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <form method="POST" action="">
            <div class="mb-4 ">
                ...
                <input type="text" name="nom" id="nom" value="<?php echo $player_data['nom']?>" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" required>
            </div>
            <div class="mb-4">
                <label for="prenom" class="block text-sm font-medium text-gray-700">Prénom</label>
                <input type="text" name="prenom" id="prenom" value="<?php echo $player_data['prenom']?>" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" required>
            </div>
            <div class="mb-4">
                <label for="date_naissance" class="block text-sm font-medium text-gray-700">Date de Naissance</label>
                <input type="date" name="date_naissance" id="date_naissance" value="<?php echo $player_data['']?>" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" required>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="hidden" name="id_joueur" value="<?php echo$_GET['id']?>"/>
            </div>
